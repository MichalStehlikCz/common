...
public final class DtDate implements Comparable<DtDate> {
  ...
  /**
   * Regular expression validating year, strict (4 digit). Valid years correspond to date values
   * allowed in Provys framework
   */
  ...
  /**
   * Retrieves instance of {@code DtDate} corresponding to given year, month and day as {@code int}
   * values.
   *
   * @param year  is year of date to be created
   * @param month is calendar month of date to be created
  ...
  public static DtDate of(int year, int month, int day) {
    return of(year, (short) month, (short) day);
  }
   ...
   */
  public static DtDate parse(StringParser parser, boolean allowSpecialText,
    ...
    try {
      ...
      if (parser.next() != '-') {
        throw new DateTimeParseException("Expecting - as year / month delimiter",
            parser.getString(),
            parser.getPos());
      }
  ...
  private static void readHourDelimiter(StringParser parser) {
    if (parser.next() != ':') {
  ...
  private static void readMinutes(StringParser parser) {
    if (parser.readUnsignedInt(2) != 0) {
      throw new DateTimeParseException("Minutes expected to equal zero in ISO date",
          parser.toString(),
          parser.getPos());
    }
  }

  private static void readSeconds(StringParser parser) {
    if (parser.hasNext() && (parser.peek() == ':')) {
      // seconds
      parser.next();
      if (parser.readUnsignedInt(2) != 0) {
        throw new DateTimeParseException("Seconds expected to equal zero in ISO date",
            parser.toString(),
            parser.getPos());
      }
  ...
  private static DtDate parseIsoTime(DtDate date, StringParser parser) {
    ...
    if (parser.hasNext() && (parser.peek() == 'T')) {
      ...
      if (hours == 24) {
        ...
        throw new DateTimeParseException("Hours expected to equal zero in ISO date",
            parser.toString(),
            parser.getPos());
      }
      readHourDelimiter(parser);
      readMinutes(parser);
      readSeconds(parser);
    }
    return result;
   ...
   *                  also accepts date with zero time or with zero time with timezone
   * @param allowTime defines if time following date information is to be parsed. If so, it parses
   *                  time and throws exception if it is different from zero or 24 hours
   * @return datetime value read from parser
   */
  public static DtDate parseIso(StringParser parser, boolean allowTime) {
    ...
    try {
      ...
      throw new DateTimeParseException("Unexpected end of string encountered in ISO date",
   ...
   *             zero time or with zero time with timezone
   * @return date value corresponding to provided text
   */
  public static DtDate parseIso(String text) {
    ...
    if (parser.hasNext()) {
      throw new DateTimeParseException("ISO date value parsed before reading whole text", text,
          parser.getPos());
    }
    return result;
  }
  ...
  /**
   * Parse provided text as Provys string representation of date value, in format DD.MM.YYYY. It
   * also accepts value with time part 00:00:00. It can finish reading before reading whole content
   * of parser.
   *
  ...
  public static DtDate ofProvysValue(StringParser parser) {
    try {
      ...
      throw new DateTimeParseException("Error reading date value", parser.getString(),
          parser.getPos(), e);
    }
  }
  ...
  /**
   * Parse provided text as Provys string representation of date value, in format DD.MM.YYYY. It
   * also accepts value with time part 00:00:00.
   *
  ...
  private DtDate(LocalDate value) {
    ...
    if (value.compareTo(MINVALUE) < 0) {
      throw new DateTimeException(
          value.toString() + " is not valid date - cannot be smaller than " + MINVALUE);
    }
    if (value.compareTo(MAXVALUE) > 0) {
      throw new DateTimeException(
          value.toString() + " is not valid date - cannot be bigger than " + MINVALUE);
    }
    this.value = value;
  }
  ...
  /**
   * Returns a copy of this {@code DtDate} with the specified number of days added. This method adds
   * the specified amount of days and ensures result remains valid. The result is only invalid if
   * the maximum/minimum year is exceeded. When function is applied on special value (MIN, MAX,
  ...
  /**
   * Converts {@code DtDate} value to ISO date string representation. Unlike toString, special
   * values are also converted to normal Iso date format, as special strings would not pass document
   * validation
   *
